...
def quickSort(numbers_list):
    if len(numbers_list) <= 1:
        ...
        less_than_pivot = [x for x in numbers_list[1:] if x <= pivot]
        greater_than_pivot = [x for x in numbers_list[1:] if x > pivot]
        return quickSort(less_than_pivot) + [pivot] + quickSort(greater_than_pivot)


def main():
    run_sorting_algorithm("quickSort", numbers_list)
